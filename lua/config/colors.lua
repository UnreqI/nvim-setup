...
function SetTheme(name)
    ...
    vim.g.current_theme = name -- store current theme
...
vim.api.nvim_create_autocmd("User", {
    pattern = "LazyDone",
    callback = function()
        if vim.g.current_theme then -- restore previous theme on startup
            SetTheme(vim.g.current_theme)
        else
            SetTheme("catppuccin") -- fallback default
        end
    end,
})
